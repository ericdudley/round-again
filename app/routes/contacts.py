...
def detail(contact_id):
    ...
    try:
        ...
        interactions = session.query(Interaction).filter_by(contact_id=contact_id).order_by(Interaction.interaction_date.desc()).all()
        
        return render_template('contact_detail.html', contact=contact, interactions=interactions, now=now)
    finally:
        session.close()
...
def edit_form(contact_id):
    ...
    try:
        ...
        frequency_units = FrequencyUnit.__members__.values()
        return render_template('contact_form.html', contact=contact, frequency_units=frequency_units)
    finally:
        session.close()
...
def new_interaction_form(contact_id):
    ...
    try:
        ...
        return render_template('interaction_form.html', contact=contact, interaction_types=interaction_types)
    finally:
        session.close()